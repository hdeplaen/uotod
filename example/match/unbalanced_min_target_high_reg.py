...
M_softmin = uotod.match.SoftMin(loc_match_module=L, background_cost=0.8, reg=0.1, source='target')
M_unb_small = uotod.match.UnbalancedSinkhorn(loc_match_module=L, background_cost=0.8, reg=0.1, reg_pred=1.e-2, reg_target=1.e+4)
M_unb_med = uotod.match.UnbalancedSinkhorn(loc_match_module=L, background_cost=0.8, reg=0.1, reg_pred=.2, reg_target=1.e+4)
...
matches = [M_softmin(input, target)[0, :, :],
...
fig_matches = uotod.plot.multiple_matches(matches=matches,
                                          subtitles=['SoftMin from\nthe targets',
                                                     ...
                                                     'Unbalanced Sink.\n(high reg_pred)',
                                                     'Balanced\nSinkhorn'],
                                          title='Effect of reg_pred (reg=0.1)',
                                          figsize=(20, 6))
fig_matches.show()
